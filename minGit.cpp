...
void minGit::checkout(int fileNumber){
        ...
        if(curr->commitNumber == fileNumber)
        {
            singlyNode* latest = curr->head;
                while(latest != NULL)
                {
                    ...
                    ifstream file1;
                    fileOverwrite.open(node->fileName, std::ofstream::trunc);
    ...
    bool menCh = false;
char choice;
cout << " In the previous version in the repository.Warning !!! canoot add, commit or delete...type y to return "<< endl;
